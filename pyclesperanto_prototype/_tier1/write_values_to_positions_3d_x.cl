...
)
{
  ...
  const int z = READ_src_IMAGE(src,sampler, (sourcePos + POS_src_INSTANCE(0, 2, 0, 0))).x;
  const float v = READ_src_IMAGE(src,sampler, (sourcePos + POS_src_INSTANCE(0, 3, 0, 0))).x;
