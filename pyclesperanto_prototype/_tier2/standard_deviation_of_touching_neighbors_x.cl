...
__kernel void standard_deviation_value_of_touching_neighbors (
  ...
  for (x = 0; x < label_id; x++) {
    float value = READ_IMAGE(src_touch_matrix, sampler, POS_src_touch_matrix_INSTANCE(x, y, 0, 0)).x;
    if (value > 0) {
      value = READ_IMAGE(src_values, sampler, POS_src_values_INSTANCE(x + x_correction, 0, 0, 0)).x - average;
      sum = sum + value * value;
      count++;
    }
  }
  ...
  // assume the object is included in mean calculation
  float value1 = READ_IMAGE(src_values, sampler, POS_src_values_INSTANCE(label_id + x_correction, 0, 0, 0)).x - average;
  ...
  for (y = label_id + 1; y < label_count; y++) {
    float value = READ_IMAGE(src_touch_matrix, sampler, POS_src_touch_matrix_INSTANCE(x, y, 0, 0)).x;
    if (value > 0) {
      value = READ_IMAGE(src_values, sampler, POS_src_values_INSTANCE(y + x_correction, 0, 0, 0)).x - average;
      sum = sum + value * value;
      count++;
    }
  }
  ...
  WRITE_IMAGE(dst_values, POS_dst_values_INSTANCE(label_id, 0, 0, 0), CONVERT_dst_values_PIXEL_TYPE(stdDev));
}
